...
export const GamePage: FC = () => {
  ...
  return (
    <div className={`flex-1 flex flex-col gap-6 p-6 md:p-8 transition-colors duration-300 ${
      ...
      <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4 ${
        ...
        <div className="flex items-center gap-3">
          <Link
            to="/dashboard"
            className={`p-2 border rounded-[4px] transition-all cursor-pointer ${
              dark
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className={`text-[9px] font-bold tracking-widest border px-1.5 py-0.5 rounded-[4px] uppercase ${
                dark ? 'text-[#64748b] bg-black/40 border-slate-800' : 'text-slate-550 bg-slate-100 border-slate-200'
              }`}>
                {game.category}
              </span>
              <span className="text-sm">{game.thumbnail}</span>
            </div>
            <h2 className={`text-xl font-bold tracking-wide uppercase mt-1 ${
              dark ? 'text-silver-gradient' : 'text-slate-950'
            }`}>
              {game.name}
            </h2>
          </div>
      ...
      <div className={`flex-1 flex items-center justify-center border rounded-[4px] p-4 md:p-8 shadow-inner relative overflow-hidden transition-colors ${
        dark ? 'bg-black/20 border-slate-850' : 'bg-slate-50 border-slate-200'
      }`}>
        {/* Subtle Speed lines background */}
        <div className={`absolute inset-x-0 top-1/4 h-[1px] animate-speed-streak-slow pointer-events-none ${
          dark ? 'bg-slate-900' : 'bg-slate-200'
        }`} />
        <div className={`absolute inset-x-0 bottom-1/4 h-[1px] animate-speed-streak-2 pointer-events-none ${
          dark ? 'bg-slate-900' : 'bg-slate-200'
        }`} />
      ...
      {/* Game Details Card */}
      <div className={`border rounded-[4px] p-5 flex flex-col sm:flex-row justify-between gap-6 transition-colors ${
        dark ? 'bg-[#1a1a1c] border-slate-805' : 'bg-white border-slate-200'
      }`}>
        <div className="flex-1">
          ...
          <p className={`text-xs leading-relaxed font-medium ${
            dark ? 'text-slate-400' : 'text-slate-655'
          }`}>
            {game.description}
          </p>
        </div>
        <div className={`sm:w-64 border-t sm:border-t-0 sm:border-l pt-4 sm:pt-0 sm:pl-6 shrink-0 ${
          dark ? 'border-slate-800' : 'border-slate-200'
        }`}>
