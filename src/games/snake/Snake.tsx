...
export const Snake: FC = () => {
  ...
  const handleTouchMove = (e: React.TouchEvent<HTMLCanvasElement>) => {
    if (gameStatus !== 'PLAYING' || !touchStartPos.current) return;
    const touch = e.touches[0];
    ...
    if (Math.abs(dx) > Math.abs(dy)) {
      if (Math.abs(dx) > threshold) {
        if (dx > 0) {
          handleOnscreenControl('RIGHT');
        } else {
          handleOnscreenControl('LEFT');
        }
        touchStartPos.current = { x: touch.clientX, y: touch.clientY };
      }
    } else {
      if (Math.abs(dy) > threshold) {
        if (dy > 0) {
          handleOnscreenControl('DOWN');
        } else {
          handleOnscreenControl('UP');
        }
        touchStartPos.current = { x: touch.clientX, y: touch.clientY };
      }
    }
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    if (gameStatus !== 'PLAYING' || !mouseStartPos.current) return;
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 w-full max-w-5xl mx-auto px-4 py-8 select-none">
      ...
      <div className="flex-1 flex flex-col items-center">
        ...
        <div className={`relative border rounded-[4px] overflow-hidden bg-[#09090b] w-full max-w-[480px] transition-all duration-300 ${
          ...
          <canvas
            ...
            width={400}
            height={400}
            className="block w-full aspect-square touch-none cursor-grab active:cursor-grabbing"
          ...
          {gameStatus === 'IDLE' && (
            <div className="absolute inset-0 bg-white/95 dark:bg-[#121214]/95 flex flex-col items-center justify-center p-6 text-center z-20">
              <Gamepad2 className="w-12 h-12 text-zinc-800 dark:text-[#e8e8ea] mb-3" />
              <h3 className="text-base font-bold text-zinc-900 dark:text-white mb-4 uppercase tracking-wider">Snake Arena</h3>
          ...
          {gameStatus === 'PAUSED' && (
            <div className="absolute inset-0 bg-white/95 dark:bg-[#121214]/95 flex flex-col items-center justify-center p-6 gap-4 z-20">
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white uppercase tracking-wider">Paused</h3>
          ...
          {gameStatus === 'GAME_OVER' && (
            <div className="absolute inset-0 bg-white/95 dark:bg-[#121214]/95 flex flex-col items-center justify-center p-6 text-center z-20">
              ...
              <p className="text-zinc-650 dark:text-slate-300 mb-4 font-medium">
                Final score: <span className="text-zinc-900 dark:text-white font-bold">{score}</span>
              </p>
        ...
        <div className={`mt-6 flex-col items-center w-full max-w-[200px] ${useTouchLayout ? 'flex' : 'hidden md:flex'}`}>
          <button
            onClick={() => handleOnscreenControl('UP')}
            disabled={gameStatus === 'GAME_OVER' || gameStatus === 'PAUSED'}
            className="w-12 h-12 bg-zinc-100 dark:bg-[#1a1a1c] border border-zinc-200 dark:border-slate-800 active:bg-zinc-900 dark:active:bg-white active:text-white dark:active:text-black rounded-[4px] flex items-center justify-center mb-1 text-zinc-700 dark:text-slate-300 transition-colors font-bold cursor-pointer"
          >
            ▲
          </button>
          <div className="flex gap-1">
            <button
              ...
              disabled={gameStatus === 'GAME_OVER' || gameStatus === 'PAUSED'}
              className="w-12 h-12 bg-zinc-100 dark:bg-[#1a1a1c] border border-zinc-200 dark:border-slate-800 active:bg-zinc-900 dark:active:bg-white active:text-white dark:active:text-black rounded-[4px] flex items-center justify-center text-zinc-700 dark:text-slate-300 transition-colors font-bold cursor-pointer"
            >
              ◀
            </button>
            <button
              ...
              className="w-12 h-12 bg-zinc-100 dark:bg-[#1a1a1c] border border-zinc-200 dark:border-slate-800 active:bg-zinc-900 dark:active:bg-white active:text-white dark:active:text-black rounded-[4px] flex items-center justify-center text-zinc-700 dark:text-slate-400 text-xs transition-colors cursor-pointer"
            >
          ...
          <button
            ...
            disabled={gameStatus === 'GAME_OVER' || gameStatus === 'PAUSED'}
            className="w-12 h-12 bg-zinc-100 dark:bg-[#1a1a1c] border border-zinc-200 dark:border-slate-800 active:bg-zinc-900 dark:active:bg-white active:text-white dark:active:text-black rounded-[4px] flex items-center justify-center mt-1 text-zinc-700 dark:text-slate-300 transition-colors font-bold cursor-pointer"
          >
            ▼
          </button>
        </div>
      ...
      <div className="w-full lg:w-80 flex flex-col gap-6">
        ...
        <div className="bg-zinc-100 dark:bg-[#1a1a1c] border border-zinc-200 dark:border-slate-800 rounded-[4px] p-6 flex flex-col gap-5">
          <h3 className="text-xs font-bold text-zinc-800 dark:text-white uppercase tracking-wider flex items-center gap-1.5">
            <Settings className="w-4 h-4 text-zinc-550 dark:text-slate-400" /> Game Options
          </h3>
          ...
          <div>
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 w-full py-1.5 rounded-[4px] border text-[9px] font-bold transition-all cursor-pointer bg-zinc-200/50 dark:bg-black/30 border-zinc-300 dark:border-slate-800 text-zinc-650 dark:text-slate-400 hover:border-zinc-400 dark:hover:border-slate-500 hover:text-zinc-955 dark:hover:text-white"
            >
              {muted ? (
                <>
          ...
          <div>
            <div className="text-[10px] font-bold text-zinc-500 dark:text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-1">
              <Grid className="w-3 h-3 text-zinc-500 dark:text-slate-455" /> Arena Map Layout
            </div>
                ...
                <button
                  ...
                  className={`py-1.5 px-2 rounded-[4px] border text-[10px] font-bold text-center transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed ${
                    boardModel === model.id
                      ...
                      : 'bg-zinc-200/50 dark:bg-black/30 border-zinc-300 dark:border-slate-800 text-zinc-655 dark:text-slate-400 hover:border-zinc-400 dark:hover:border-slate-605 hover:text-zinc-955 dark:hover:text-white'
                  }`}
                >
                  {model.name}
                </button>
            ...
            <div className="flex gap-1.5">
              <button
                ...
                className={`flex-1 py-1.5 rounded-[4px] border text-[9px] font-bold transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed ${
                  wallMode
                    ? 'bg-zinc-900 dark:bg-white text-white dark:text-black border-zinc-900 dark:border-white'
                    : 'bg-zinc-200/50 dark:bg-black/30 border-zinc-300 dark:border-slate-800 text-zinc-650 dark:text-slate-400 hover:border-zinc-400 dark:hover:border-slate-500 hover:text-zinc-950 dark:hover:text-white'
                }`}
              >
                Solid Walls
              </button>
          ...
          <div className="border-t border-zinc-200 dark:border-slate-800 pt-3">
            <span className="text-[10px] font-bold text-zinc-500 dark:text-slate-400 uppercase tracking-wider block mb-2">
              Board Items
            </span>
            <div className="space-y-2 text-[10px] font-semibold text-zinc-655 dark:text-slate-400">
          ...
          {useTouchLayout ? (
            <ul className="text-xs space-y-3 text-zinc-655 dark:text-slate-400">
              <li className="flex justify-between items-center border-b border-zinc-200 dark:border-zinc-900 pb-2">
                <span>Steer / Change Direction</span>
                <span className="text-[10px] font-bold text-zinc-700 dark:text-zinc-300">Tap Arrow Buttons (▲, ◀, ▼, ▶)</span>
              </li>
              <li className="flex justify-between items-center border-b border-zinc-200 dark:border-zinc-900 pb-2">
            ...
            <ul className="text-xs space-y-3 text-zinc-655 dark:text-slate-400">
              <li className="flex justify-between items-center border-b border-zinc-200 dark:border-zinc-900 pb-2">
                <span>Move Up</span>
                <kbd className="bg-zinc-200 dark:bg-black/50 border border-zinc-300 dark:border-slate-800 px-2 py-0.5 rounded text-zinc-800 dark:text-[#e8e8ea] text-[10px] font-mono">W / ▲</kbd>
              </li>
              <li className="flex justify-between items-center border-b border-zinc-200 dark:border-zinc-900 pb-2">
                <span>Move Left</span>
                <kbd className="bg-zinc-200 dark:bg-black/50 border border-zinc-300 dark:border-slate-800 px-2 py-0.5 rounded text-zinc-800 dark:text-[#e8e8ea] text-[10px] font-mono">A / ◀</kbd>
              </li>
              <li className="flex justify-between items-center border-b border-zinc-200 dark:border-zinc-900 pb-2">
                <span>Move Down</span>
                <kbd className="bg-zinc-200 dark:bg-black/50 border border-zinc-300 dark:border-slate-800 px-2 py-0.5 rounded text-zinc-800 dark:text-[#e8e8ea] text-[10px] font-mono">S / ▼</kbd>
              </li>
              <li className="flex justify-between items-center border-b border-zinc-200 dark:border-zinc-900 pb-2">
                <span>Move Right</span>
                <kbd className="bg-zinc-200 dark:bg-black/50 border border-zinc-300 dark:border-slate-800 px-2 py-0.5 rounded text-zinc-800 dark:text-[#e8e8ea] text-[10px] font-mono">D / ▶</kbd>
              </li>
        ...
        <div className="bg-zinc-100 dark:bg-[#1a1a1c] border border-zinc-200 dark:border-slate-800 rounded-[4px] p-6 flex-1 flex flex-col">
          <h3 className="text-xs font-bold text-zinc-800 dark:text-white uppercase tracking-wider mb-4 flex items-center gap-2">
            <Award className="w-4 h-4 text-zinc-550 dark:text-slate-500" /> Leaderboard Logs
          </h3>
          <div className="flex-1 overflow-y-auto max-h-[250px] space-y-2 pr-1">
            {leaderboard.length === 0 ? (
              ...
              leaderboard.slice(0, 5).map((entry, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between py-2 px-3 bg-zinc-200/60 dark:bg-black/20 border border-zinc-300 dark:border-slate-855 rounded-[4px]"
                >
